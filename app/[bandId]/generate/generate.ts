...
export async function generate(params: { repertoire: Song[]; prompt: string }) {
  ...
  You generally want about 8 songs in a setlist. Each song is usually around 3-4 min long, so adjust the number of songs accordingly if the user provides a length of time for the set.
  ...
  Your response should be an array of objects with the fields "title" and "key". 
  Here is the song repertoire in JSON format: ${JSON.stringify(repertoire)}
  `;
